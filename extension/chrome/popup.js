document.getElementById('addBtn').addEventListener('click', async () => {
  try {
    ...
    chrome.tabs.sendMessage(tab.id, { type: 'GET_PAGE_METADATA' }, (metadata) => {
...
async function saveTags(newTags) {
  const result = await chrome.storage.local.get('usedTags');
  const usedTags = new Set(result.usedTags || []);
  newTags.forEach(tag => usedTags.add(tag));
  await chrome.storage.local.set({ usedTags: Array.from(usedTags) });
...
document.getElementById('manageBtn').addEventListener('click', async () => {
  ...
  if (managerTab) {
    chrome.tabs.update(managerTab.id, { active: true });
    chrome.windows.update(managerTab.windowId, { focused: true });
  } else {
